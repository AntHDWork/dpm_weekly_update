// api/combine.js
// Minimal stub endpoint for Weekly Update Combiner.
// Accepts POST { run_metadata: { week_ending }, submissions: [ { project_key, dpm, status, delta, ... } ] }
// Returns JSON { executive_summary_md, combined_update_md }.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const s of submissions) {
      if (!s || !s.project_key) continue;
      byKey[s.project_key] = s;
    }
    ...
    // Very simple “notables” from deltas (stub logic)
    const notable = [];
